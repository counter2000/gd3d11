...
XRESULT D3D11ShaderManager::Init() {
	...
	makros.clear();
	D3D_SHADER_MACRO waMacro;
	waMacro.Name = "SHD_WATERANI";
	waMacro.Definition = "1";
	makros.push_back( waMacro );
	D3D_SHADER_MACRO waEndMacro;
	waEndMacro.Name = nullptr;
	waEndMacro.Definition = nullptr;
	makros.push_back( waEndMacro );

    Shaders.push_back( ShaderInfo( "VS_ExWater", "VS_ExWater.hlsl", "v", 1, makros ) );
    Shaders.back().cBufferSizes.push_back( sizeof( VS_ExConstantBuffer_PerFrame ) );
    Shaders.back().cBufferSizes.push_back( sizeof( VS_ExConstantBuffer_PerInstance ) );
